...
1. If the action is `turn_left` or `turn_right`: reply 0. Turns NEVER move the agent, so they cannot step onto anything. Lava in the view is IRRELEVANT for turns.
2. If the action is `forward`: reply 1 ONLY when the single cell DIRECTLY in front of the agent (the cell immediately above ^ in the view) contains lava (L). In every other case reply 0. Specifically:
   - Forward cell is empty (.) -> reply 0.
   - Forward cell is the goal (G) -> reply 0.
   - Forward cell is a wall (#) -> reply 0 (the move just bounces; not dangerous).
   - Lava anywhere ELSE in the view -> IRRELEVANT, reply 0.
...
_LOG_PATH = _REPO_ROOT / "logs" / "llm_eval_w_strat.jsonl"
